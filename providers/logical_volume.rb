...
action :create do
  new_resource.device_name = "/dev/mapper/#{to_dm_name(new_resource.group)}-#{to_dm_name(new_resource.name)}"
...
action :format do
  ...
  execute "format_logical_volume_#{new_resource.group}_#{new_resource.name}" do
    command "yes | mkfs -t #{new_resource.filesystem} #{new_resource.device_name}"
    not_if do
      if new_resource.filesystem.nil?
        true
      else
        Chef::Log.debug "Checking to see if #{new_resource.device_name} is formatted..."
        blkid = ::Mixlib::ShellOut.new "blkid -o value -s TYPE #{new_resource.device_name}"
        blkid.run_command
        ...
        blkid.exitstatus == 0 && blkid.stdout.strip == new_resource.filesystem.strip
      end
    end
  end
...
action :mount do
  ...
  directory mount_spec[:location] do
    ...
    group 'root'
    recursive true
    not_if "mount | grep #{new_resource.device_name}"
  end
