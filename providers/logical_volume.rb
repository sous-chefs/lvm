...
action :create do # rubocop:disable MethodLength
  require 'lvm'
  lvm = LVM::LVM.new
  ...
  if vg.nil? || vg.logical_volumes.select { |lv| lv.name == name }.empty?
    ...
    lv = vg.logical_volumes.select { |lvol| lvol.name == name }.first
...
action :resize do
  ...
  if resize_type == 'byte' || resize_type == 'extent'
    lv_size_req = case lv_size
                  when /^(\d+)(k|K)$/
                    (Regexp.last_match[1].to_i * 1_024) / pe_size
                  when /^(\d+)(m|M)$/
                  ...
                  when /^(\d+)$/
                    ...
                    Chef::Application.fatal!("Invalid size #{lv_size} for lvm resize", 2)
                  end
